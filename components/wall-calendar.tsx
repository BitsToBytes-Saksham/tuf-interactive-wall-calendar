...
export default function WallCalendar() {
  ...
  const isRowStart = useCallback((index: number): boolean => index % 7 === 0, [])
  const isRowEnd = useCallback((index: number): boolean => index % 7 === 6, [])
  ...
  // ✅ FIXED: Read directly from localStorage on save, never rely on stale state
  const saveNotes = useCallback(() => {
    ...
    try {
      ...
      savedNotesRef.current = existing
  ...
  }, [currentMonth, currentYear, notes])
      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 relative">
          ...
          <div className="bg-white flex items-center justify-between px-4 py-3 border-b border-gray-100">
            <button onClick={() => navigateMonth("prev")} className="p-2 rounded-full hover:bg-gray-200 active:bg-gray-300 transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center group" aria-label="Previous month">
            ...
            <button onClick={() => navigateMonth("next")} className="p-2 rounded-full hover:bg-gray-200 active:bg-gray-300 transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center group" aria-label="Next month">
          ...
          <div className={`bg-white rounded-b-2xl shadow-xl overflow-hidden transition-all duration-300 ${isAnimating ? (animationDirection === "left" ? "translate-x-[-10px] opacity-50" : "translate-x-[10px] opacity-50") : ""}`}>
            <div className="grid grid-cols-7 bg-gradient-to-r from-slate-50 to-gray-50">
              {DAY_NAMES.map((day, i) => (
                <div key={day} className={`py-3 text-center text-xs font-semibold tracking-wider ${i === 0 ? "text-rose-500" : i === 6 ? "text-blue-500" : "text-gray-500"}`}>
                  {day}
                </div>
              ))}
            </div>
          ...
          {tooltipInfo && (
            <div className="fixed z-50 bg-gray-900 text-white text-xs px-3 py-1.5 rounded-lg shadow-lg pointer-events-none transform -translate-x-1/2 -translate-y-full" style={{ left: tooltipInfo.x, top: tooltipInfo.y }}>
              {tooltipInfo.text}
